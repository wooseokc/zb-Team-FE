...
export default function MyPage () {
  ...
  const onSubmit = async (e: React.MouseEvent<HTMLDivElement>) => {
    console.log('wait')
    await axios.get('http://34.168.232.38:8080/minesweeper/game/1').then(res => {
  ...
  const onPost = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    await axios.post(`http://34.168.232.38:8080/minesweeper/gamer/`, JSON.stringify(data)).then(res => {
